...
/**
 * @brief 以接收队列订阅精确 topic. 若 topic 未注册则注册, 将 recv_queue 加入订阅者列表.
...
/**
 * @brief 从 topic 取消队列订阅. 移除订阅并释放内部项.
 * @param topic_str   topic 字符串.
 * @param recv_queue  曾传入 epx_subscribe_queue 的队列.
...
/**
 * @brief 向 topic 发布原始负载. 分配 msg, 拷贝数据, 向各队列订阅者发送指针, 再释放发布者引用.
...
/**
 * @brief 查询 Broker 是否处于空闲 (当前无 epx_publish_data 路径在执行). 尽力检测, 不替代显式同步;
